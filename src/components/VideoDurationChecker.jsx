...
import './VideoDurationChecker.css';

function VideoDurationChecker() {
  ...
  const handleCheck = async () => {
    ...
    try {
      const response = await fetch('http://localhost:3001/api/check-video-duration', {
        method: 'POST',
        headers: {
  ...
  const downloadReport = () => {
    ...
    let reportContent = 'Video Files Duration Report\n';
    ...
    a.href = url;
    a.download = `video-duration-report-${timestamp}.txt`;
  ...
  return (
    <div className="video-checker-container">
      <div className="video-checker-header">
        <h2>🎬 Video Duration Checker</h2>
        <p className="video-checker-subtitle">Check VIDEO files for duration issues (&lt; 9s or &gt; 20s)</p>
      </div>
      ...
      {!isChecking && results.totalFiles === 0 && (
        <div className="info-section">
          <div className="info-card">
            ...
            <ul>
              <li>Select folder containing VIDEO files</li>
              <li>Tool scans all video files in the folder</li>
              <li>Extracts actual duration from each video using ffprobe</li>
...
export default VideoDurationChecker;
